...
public class CameraEncoder implements SurfaceTexture.OnFrameAvailableListener, Runnable {
    ...
    private void init(SessionConfig config) {
        ...
        Log.i("SessionConfig", config.getVideoWidth() + " X "+config.getVideoHeight());
    }
...
//            Log.i(TAG, "Device support preview size : "+ size.width+ " x "+size.height);
        ...
        if (ppsfv != null) {
            Log.i(TAG, "so we use preview size : "+ ppsfv.width+ " x "+ppsfv.height);
    ...
    private void handleSetSurfaceTexture(int textureId) throws IOException {
        synchronized (mSurfaceTextureFence) {
            if (mSurfaceTexture != null) {
                ...
                if (VERBOSE)
                    Log.i(TAG + "-SrfcTxtur", " SurfaceTexture created. pre setOnFrameAvailableListener");
